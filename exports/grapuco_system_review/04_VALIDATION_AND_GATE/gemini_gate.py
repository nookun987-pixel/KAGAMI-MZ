...
def final_gate(image_path: str, pixel_report: dict, job_dir: Path, lane: str = "mask") -> dict:
    ...
    if not path.exists():
        return {"gemini_executed": False, "parse_ok": False, "pass_fail": "FAIL",
                "final_decision": "REJECT", "error": f"Image not found: {image_path}"}
...
Be strict on identity. No drift."""
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            return {"gemini_executed": False, "parse_ok": False, "pass_fail": "FAIL",
                    "final_decision": "REJECT", "error": f"Gemini HTTP {r.status_code}"}
        ...
        try:
            ...
            return {"gemini_executed": True, "parse_ok": False, "pass_fail": "FAIL",
                    "final_decision": "REJECT", "error": f"JSON parse: {e}", "raw_text": text[:500]}

    except Exception as e:
        log.error(f"Gemini gate error: {e}")
        return {"gemini_executed": False, "parse_ok": False, "pass_fail": "FAIL",
                "final_decision": "REJECT", "error": str(e)}
