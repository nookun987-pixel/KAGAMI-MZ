/**
 * P0-6: BASELINE CLEAN OBJECT-FORM TEST
 * End-to-End test with NO LoRA, explicit industrial object prompt
 * Verify: manufactured object readability without plastic/neon/toy drift
 */
...
const TEST_JOB_ID = `e2e_test_clean_baseline_${Date.now()}`;
const JOBS_DIR = path.join(process.cwd(), 'jobs');
const RUNS_DIR = path.join(process.cwd(), 'runs');

console.log('=== P0-6: BASELINE CLEAN OBJECT-FORM TEST ===');
...
// Create test job - NO LoRA, explicit industrial object form
const jobData = {
  job_id: TEST_JOB_ID,
  ...
  render: {
    ...
    steps: 40,
    guidance_scale: 7,
    // NO LoRA - clean baseline
    lora_name: null,
    lora_weight: 0,
    style_selections: [],
    // Enhanced negative for anti-plastic/anti-toy/anti-neon
...
console.log('📝 Configuration: NO LoRA, explicit industrial object form');
console.log('📝 Enhanced negative: anti-plastic / anti-toy / anti-neon');

// Run orchestrator
console.log('\n🚀 Starting orchestrator (baseline clean object-form)...\n');
...
child.on('close', (code) => {
  ...
  // Final Verdict
  console.log('\n' + '='.repeat(60));
  console.log('FINAL VERDICT - BASELINE CLEAN OBJECT-FORM (NO LoRA)');
