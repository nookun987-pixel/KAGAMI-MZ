/**
 * P8-6 CASE C: End-to-End Test with Product Object LoRA
 * Create a test job and run through full pipeline with Product Object LoRA
 * Verify: output.png exists, local validator PASS, gemini PASS, final_decision = ALLOW
 * 
 * PATH: Low-weight product_shot LoRA (0.4) for object readability without color drift
 */
...
const TEST_JOB_ID = `e2e_test_product_object_${Date.now()}`;
const JOBS_DIR = path.join(process.cwd(), 'jobs');
const RUNS_DIR = path.join(process.cwd(), 'runs');

console.log('=== P8-6 CASE C: END-TO-END TEST WITH PRODUCT OBJECT LORA ===');
...
// Create test job WITH Product Object LoRA (low weight)
const jobData = {
  job_id: TEST_JOB_ID,
  ...
  render: {
    ...
    steps: 40,
    guidance_scale: 7,
    // Product Object LoRA configuration - LOW WEIGHT to avoid overpowering
    lora_name: "product_object.safetensors",
    lora_weight: 0.4
  }
};
...
console.log('📝 LoRA config: product_object.safetensors @ 0.4 (low weight)');

// Run orchestrator
console.log('\n🚀 Starting orchestrator with Product Object LoRA...\n');
...
child.on('close', (code) => {
  ...
  // Final Verdict
  console.log('\n' + '='.repeat(50));
  console.log('FINAL VERDICT - CASE C (PRODUCT OBJECT LORA @ 0.4)');
  ...
  if (passedChecks === totalChecks) {
    console.log('🟢 ALL CHECKS PASSED - PRODUCT OBJECT LORA TEST SUCCESS');
    console.log(`\n✅ Test run: ${TEST_JOB_ID}`);
    console.log(`✅ Final decision: ALLOW`);
    console.log(`✅ Gemini: PASS`);
    console.log(`✅ Output: EXISTS`);
    console.log(`📝 LoRA: product_object.safetensors @ 0.4`);
    process.exit(0);
  } else {
