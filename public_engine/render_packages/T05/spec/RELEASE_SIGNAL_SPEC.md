# RELEASE_SIGNAL_SPEC

PACKAGE_STANDARD: MIKAGE_PUBLIC_ENGINE_OPERATING_STANDARD_V1
TRACK_ID: T05
TRACK_TITLE: PORCELAIN ASCENSION
AUDIO_WINDOW: 0:00 -> 0:30 / 0:33 -> 1:17
TARGET_OUTPUT: RELEASE_SIGNAL_SHORT_V2
VIDEO_FORMAT: 1080x1920 vertical short
VISUAL_RULE: Text-only; no approved optional assets in Track Package.
SCREEN_RULE: 5-sub means sequential timed text blocks or production manifest, not five static lines on screen.
BANNED: No MP4 render; no video output; no hook timeline change; no creative rewrite; no invented live status; no invented proof status; no invented website status; no lyrics; no cover; no character art; no Listen now CTA without confirmed live status.
CTA_ON_SCREEN: Pre-save:
CTA_CAPTION: Pre-save:
MISSING_ITEMS: AUDIO_FILE; CAPTION_SOURCE_STATUS; TIMED_TEXT_STATUS; PRE_SAVE_LINK; LIVE_LINK; RELEASE_DATE; TOOLOST_STATUS; LINK_STATUS; LANGUAGE; EXPLICIT_STATUS; PROOF_PACK_STATUS; WEBSITE_STATUS
