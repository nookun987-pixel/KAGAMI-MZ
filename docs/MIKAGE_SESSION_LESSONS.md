...
## 2026-08-07 — Zenith Blade paperwork round: three failure modes worth naming

> Appended by `ZENITH_BLADE_DOCTRINE_ISSUE_01` under explicit operator authorization (append-only,
> that task only). Second authorized append; the first was `ZENITH_BLADE_PAPERWORK_CLOSEOUT_01`.

- **2026-08-07 · Fabricated hashes — caught before landing (`ZENITH_BLADE_PAPERWORK_TIDY_01`).** While drafting the promotion-packet superseded note I wrote two **invented placeholder sha256 values** — for the packet file and the control map — instead of computing them, intending to fill them in later. I caught it on self-review, computed the real values (`05f85f56…68f7`, `08dc280f…aad7`), replaced both, and asserted the placeholders were gone before the manifest or validator ever read them. RULE: **never write a hash you have not computed, not even as a placeholder.** A frozen-record system's entire value is that its hashes are real; one invented value silently converts a byte-frozen guarantee into a lie, and a later reader has no way to tell which values were measured. If a hash is not available yet, write `TO BE COMPUTED` in capitals — never hex-shaped filler, which looks exactly like a real value.
- **2026-08-07 · Diagram-parameter leak — a recurring pattern, not a one-off.** Three separate times in this arc, **visualization parameters migrated into lore/spec through AI retelling** and had to be caught: (1) the MAT_C2 "kintsugi-gold tip" that was a **MATERIAL_ID false-colour index map**, not a material; (2) the OUT3 **explode offsets** (0.48/0.36/0.24/0.12 blade-width multipliers, 0.056805 m step) which are diagram layer-spacing, presented as if mechanical properties of the weapon; (3) the **≤5% global-violet render-QA gate** threshold, presented as physical behaviour of the core (measured value is 0.5005%, and it is a pixel-coverage gate, nothing more). RULE: **every documentation round must explicitly screen for diagram/QA/diagnostic parameters before they reach a spec sentence.** Ask of every number: *is this a property of the OBJECT, or a property of the PICTURE OF the object?* ID colours, AOV/index renders, explode offsets, gate thresholds, camera and exposure settings, ROI pixel counts — all are properties of the picture. They must be labelled as such at the moment they are first written down, because once they are in prose they read as fact.
- **2026-08-07 · "Point-down" — a term surviving three retellings while contradicting locked canon.** The phrase "point-down" entered at the V0.1 cine mock (2026-06-28), was carried into the 2026-07-05 Codex slab brief §2 ("Point DOWN is the resting orientation"), and arrived in the 2026-08-07 external doctrine draft as an operational rule with three cited bases — **none of which contained it**. `MIKAGE_ZENITH_CANON_V2.md` §2.4 has no orientation statement at all; `MIKAGE_ZENITH_BLADE_SPEC_V1.md` §3 says the opposite ("**No pointed tip**", "decisive flat-cut top"); the board panels are renders whose run-logs carry only camera/scene metadata. CE15 has **no point** — its termination is blunt mechanical per the BRUTALIST CONVERGENCE ruling. Operator ruling R1 (2026-08-07) retired the wording permanently; the rule is **"vertical carry, termination down"** (basis `SPEC_V1:113`). RULE: **verify wording against the spec, never against prior prose.** A term that has been repeated by three successive AI passes has accumulated confidence but not evidence; the repetition is the danger signal, not the credential. When a draft hands you a citation, open the cited line — the citation being *offered* is not the citation being *true*.

## 2026-08-07 — CE15 actor collision: rest pose is not the actor's neutral

> Appended by `CE15_ACTOR_COLLISION_REPROOF_04` / `REPROOF_05 PRE-D0` under explicit operator
> authorization (append-only, that task only). Third authorized append.

- **2026-08-07 · Never assume the armature rest pose is the actor's visual neutral.** PRE-D0 was built on that assumption and failed: resetting every pose bone with `matrix_basis.identity()` moved `A2_right_porcelain_mitten_hand_attached_read` **0.492471337 m** while the other 25 meshes held at exactly `0.000000`. The reset itself was correct (max bone deviation from rest `7.5e-08 m`). RULE: **rest pose is internal structural data, not a geometric claim about the actor.** Before using rest as a baseline, measure the actor at rest against its canon bbox — if they disagree, rest is not the neutral and something is anchored to the posed state.
- **2026-08-07 · Pre-existing asymmetric bone parenting can make canon geometry depend on constrained evaluation.** V0.89 bone-parents the RIGHT mitten to `hand.R` (`parent_type=BONE`) while the LEFT mitten is parented to an intent empty (`parent_type=OBJECT`). Because of that one asymmetry, the actor's canon-verified bbox `[0.871986, 0.230554, 1.753685]` is only reproduced while the 15 constraints are live. RULE: **audit parent_type per object, not per group.** A fixture is not uniform just because its naming is; one bone-parented member is enough to couple the whole canon measurement to the constraint layer. Do not repair this by editing rest data — that changes the rig source.
- **2026-08-07 · Do not let `hide_render=True` meshes into an articulation MINIMUM.** The first articulation gate read `driven_min = 0.000000` and I recorded it as a failure. The zero came from `A2_blade_material_dark_front_weight_refinement`, which is render-disabled and therefore excluded from measurement by `MIKAGE_COLLISION_METHOD_V1` §2.1. With the exclusion applied, the same gate passes on all three render-enabled driven meshes. RULE: **apply the standard's exclusion set to EVERY statistic, not just to the collision count** — a min/max over a set that includes excluded objects silently reports the excluded object's value.
- **2026-08-07 · Capture the neutral BEFORE muting, never after.** The working procedure is: capture the evaluated constrained-neutral bone matrices *with the constraints still active*, then mute, then reconstruct into `pose_bone.matrix_basis` parent-before-child. RULE: **never write this as the shorthand "mute → neutral"** — that ordering is wrong and the shorthand hides it. Once the constraints are muted the neutral no longer exists to be captured.
- **2026-08-07 · Bind only after the collision neutral is reconstructed and verified.** Binding while the constraints were live anchored the meshes to the scrambled skeleton; un-muting with the meshes bound displaced the actor **2.285531 m** on a 1.753685 m figure. RULE: **bind last.** Any rigid bind freezes the relationship that exists at that instant, so the fixture must already be in its final neutral before a single parent is set.
- **2026-08-07 · Two harness traps worth carrying forward.** (1) The staging file links two libraries sharing **14 object names** — both armatures, all 8 `*_ctrl`, the IK target — so `bpy.data.objects[name]` is ambiguous; every lookup must match **name + library** and abort on anything other than exactly one hit. (2) The actor enters the scene as a **collection instance** through `ACTOR_CANON_SCALE_ROOT` (scale `0.245271`, zero parented children); iterating `bpy.data.objects` returns the un-instanced originals and I mis-measured the actor as `7.15 m` and wrongly reported the staging file as mis-scaled. RULE: **measure through `depsgraph.object_instances`, exactly as `METHOD_V1` §2.3 says** — the guard exists because this defect has now occurred three times.
