...
LATEST_COMPLETED_TASK = PREPARE_LIMITED_INTERNAL_FOLLOW_UP_PLANNING_GATE_FROM_LIMITED_DOWNSTREAM_DECISION_WORK_V0_1
...
## NEXT SAFE TASK
NEXT_SAFE_TASK = REVIEW_LIMITED_INTERNAL_FOLLOW_UP_PLANNING_GATE_FROM_LIMITED_DOWNSTREAM_DECISION_WORK_V0_1

## NEXT TASK RULES
The next task must review the limited internal follow-up planning gate for the character asset phase before reopening any character asset production work.

The next task may review internal planning documentation only. It must not modify locked source blend, modify derivative blend, create renders, edit PNG files, create public output, deploy website/social assets, touch the Public Engine or GPT Web shortcut lane, make cinematic readiness claims, make final trailer claims, make public readiness claims, approve diagnostic stills as public assets, or make character completion claims.
