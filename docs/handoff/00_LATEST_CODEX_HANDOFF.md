...
SESSION_RESUME_NOTE = docs/handoff/SESSION_RESUME_NOTE_20260601.md (READ FIRST in a new chat: full session recap + reusable RunPod/ControlNet recipe + open items; pod TERMINATED, models gone, re-setup needed next time)
...
ZENITH_BLADE_STATE_MODEL = REST non-combat = ornate mechanical blade (MJ design; refs Drive MIKAGE/zenith + zenith V2) flux-pinned to back (full weapon, NOT mini) -> COMBAT 350kg "Đại Đao 3 Pha": Pha1 Silent / Pha2 Side-channel Pulse / Pha3 Thermal Overload (per-phase wording PROVISIONAL). Forbidden-drift (curve/thin/ornate/circular) scoped to COMBAT form ONLY; non-combat ornate is canon.
ZENITH_BLADE_RUNPOD_PACKET_STATUS = EXECUTION_PACKET_READY (2026-06-01c; NOT rendered; ST0 mini REMOVED; 4 states ST1-ST4, 2 seeds/state = 8 outputs)
ZENITH_BLADE_FIGURE_FLAG = Master source says Mikage = white porcelain KITSUNE MASK + 3 LOCKED appearance variants (Imperial Clean / Fallen-Exile / Execution, storyline variants). MAY DIVERGE from recent "faceless sealed monocoque" figure work. NEEDS operator reconciliation in FIGURE LANE (not blade task; no figure files changed).
PRIOR_NEXT_TASK_DONE5 = RECORD_ZENITH_BLADE_OPERATOR_RULING_AND_REVISE_PACKET (2026-06-01b)
PRIOR_NEXT_TASK_DONE6 = RECONCILE_ZENITH_BLADE_WITH_DRIVE_MASTER_AUDIT (2026-06-01c; spec §0 audit added; compact-idle deprecated; per-phase marked provisional; packet -> ST1-ST4; Kitsune-mask figure flag raised; NO render by Claude)
CURRENT_NEXT_TASK = OPERATOR_SUPPLY_BLADE_REFS_AND_CODE_PHASES_THEN_RUN_OR_RETURN — operator (1) selects the exact non-combat ornate MJ PNG(s) from Drive zenith / zenith V2, (2) optionally codes the per-phase wording into the master to lock §2, then optionally (3) runs ZENITH_BLADE_RUNPOD_PACKET (4 states ST1-ST4) on RunPod and returns candidate PNGs for Claude to score per form. SEPARATE figure-lane item: reconcile Kitsune-mask + 3 appearance variants vs faceless work. All reference-only; NO film/video/short/shotlist; NO canon/asset-lock; NO render by Claude.
