...
MIKAGE_FULLBODY_STATUS = IN_PROGRESS (IPAdapter upper-anchor locked framing to upper body; retry V2 at low weight 0.4 for full figure; OpenPose ControlNet is the reliable fallback for true head-to-feet)
...
CURRENT_NEXT_TASK = PHASE5_UPPER_BODY_REFERENCE_SET_FILING — operator downloads the 4 adopted *_NORM.png into canon \10 and shuts down the pod. Reference set stays INCLUDE_AS_PHASE4_REFERENCE only. OPTIONAL FUTURE: 3D/Blender pass for a pixel-clean orthographic faceless SIDE/BACK turnaround (out of Cowork scope). NO film/video/short/shotlist.
