...
LATEST_COMPLETED_TASK = REVIEW_INTERNAL_DIAGNOSTIC_STILL_RENDER_SET_FROM_APPROVED_GATE_V0_1
...
## NEXT SAFE TASK
NEXT_SAFE_TASK = PREPARE_LIMITED_INTERNAL_ASSET_DECISION_GATE_FROM_DIAGNOSTIC_STILL_REVIEW_V0_1

## NEXT TASK RULES
The next task may prepare a limited internal asset decision gate from the diagnostic still review.

The next task must not create additional renders, public output, website/social deployment, final animation quality claims, cinematic readiness claims, final trailer claims, public readiness claims, or character completion claims.
