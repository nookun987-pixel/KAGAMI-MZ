...
CURRENT_NEXT_TASK = OPERATOR_REVIEW_MIKAGE_BUILD_DIRECTION_DIVERGENCES_V1 — operator supplied Mikage build-direction 2026-06-02; captured as reference note docs/handoff/MIKAGE_CHARACTER_BUILD_DIRECTION_REFERENCE_V1.md. Operator must resolve 4 divergences vs LOCKED entity spec (D1 kintsugi gold-vs-#E60000, D2 kintsugi phase-gating, D3 porcelain #f2eeea-vs-#FAFAFA, D4 Kitsune integration) + 3 UNCONFIRMED items (face-reveal policy, pre-Vessel name, mask integration). Until resolved, LOCKED MIKAGE_ZENITH_ENTITY_PHASE_SPEC_V1 stays authoritative for render briefs. Then resume OPERATOR_RUN_CAST_RUNPOD_KIT_V1 (priority cast render-ready via tools/cast_render_kit/; operator rents pod → runbook → render → hand back for Cowork verify). NO render/canon/asset-lock by Claude. See "## MIKAGE BUILD DIRECTION CAPTURE — 2026-06-02" + "## CAST RUNPOD KIT — 2026-06-02".
