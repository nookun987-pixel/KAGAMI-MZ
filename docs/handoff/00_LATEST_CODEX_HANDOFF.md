...
CURRENT_NEXT_TASK = OPERATOR_RENT_POD_AND_RUN_RUNBOOK — operator: (1) push repo (STEP 0), (2) rent 24GB ComfyUI pod, (3) paste STEP 2 (setup) + STEP 3 (fetch+run) from tools/zenith_blade_render/RUNPOD_OPERATOR_RUNBOOK.md, (4) return the 9 PNGs; Claude scores per phase (INCLUDE_AS_PHASE4_REFERENCE / HOLD / REJECT). Renders = review-candidate. PARALLEL low-pri (figure lane): verify faceless assets carry locked Kitsune mask form. NO film/video/short/shotlist; NO render by Claude.
