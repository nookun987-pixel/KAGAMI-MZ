...
LATEST_COMPLETED_TASK = REVIEW_LIMITED_INTERNAL_LIMITATION_TRACKING_GATE_FROM_FOLLOW_UP_TASK_LIST_V0_1
...
## NEXT SAFE TASK
NEXT_SAFE_TASK = PREPARE_LIMITED_INTERNAL_FOLLOW_UP_GATE_SPLIT_FROM_LIMITATION_TRACKING_REVIEW_V0_1

## NEXT TASK RULES
The next task must prepare a limited internal follow-up gate split from the approved limitation tracking review before reopening any character asset production work.

The next task may create internal planning documentation only. It must not modify locked source blend, modify derivative blend, create renders, edit PNG files, create public output, deploy website/social assets, touch the Public Engine or GPT Web shortcut lane, make cinematic readiness claims, make final trailer claims, make public readiness claims, approve diagnostic stills as public assets, open production work, or make character completion claims.
