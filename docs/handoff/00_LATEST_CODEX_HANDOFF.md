...
LATEST_COMPLETED_TASK = PREPARE_FINAL_RIG_READINESS_GATE_FROM_DIAGNOSTIC_CINEMATIC_PROOF_V0_1
...
## NEXT TASK RULES
The next task may review the prepared final rig readiness gate from the diagnostic cinematic proof milestone.

The next task must not create cinematic shots, public output, final animation claims, final rig readiness claims, cinematic readiness claims, or character completion claims.
