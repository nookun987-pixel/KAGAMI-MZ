...
ZENITH_BLADE_OPEN_FLAGS = F1 name Tri-phase=Zenith? (default SAME, CHUA_XAC_NHAN); F2 ornate blueprint on-canon/drift? (default DRIFT do-not-use, CHUA_XAC_NHAN); F3 compact-idle canon-lock+geometry? (S0=proposal only, CHUA_XAC_NHAN)
CURRENT_NEXT_TASK = OPERATOR_RULE_BLADE_FLAGS_THEN_RUN_OR_RETURN — operator (1) rules the 3 Zenith Blade open-flags (F1/F2/F3 above), then optionally (2) runs ZENITH_BLADE_RUNPOD_PACKET on RunPod and returns candidate PNGs; Claude then scores returned candidates (INCLUDE_AS_PHASE4_REFERENCE / HOLD / REJECT). Optional later: full-body proportion-refine pass. All reference-only; NO film/video/short/shotlist; NO canon/asset-lock; NO render by Claude.
