...
LATEST_COMPLETED_TASK = REVIEW_INTERNAL_DIAGNOSTIC_STILL_RENDER_GATE_FROM_PLANNING_PACKAGE_V0_1
...
## NEXT TASK RULES
The next task may create an internal diagnostic still render set from the approved gate.

The next task may create no more than 6 internal diagnostic stills and must not create public output, website/social deployment, final animation quality claims, cinematic readiness claims, final trailer claims, public readiness claims, or character completion claims.
