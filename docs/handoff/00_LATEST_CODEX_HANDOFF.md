...
LATEST_COMPLETED_TASK = REVIEW_FINAL_RIG_READINESS_GATE_FROM_DIAGNOSTIC_CINEMATIC_PROOF_V0_1
...
## NEXT TASK RULES
The next task may declare final rig readiness with limitations from the approved gate.

The next task must not create cinematic shots, public output, final animation quality claims, cinematic readiness claims, or character completion claims.
