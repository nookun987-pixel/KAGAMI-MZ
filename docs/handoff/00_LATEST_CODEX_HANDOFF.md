...
CURRENT_NEXT_TASK = OPERATOR_SUPPLY_BLADE_REFS_THEN_RUN_OR_RETURN — operator (1) selects the exact non-combat ornate MJ PNG(s) from Drive zenith / zenith V2 for the ST1 source; then optionally (2) runs ZENITH_BLADE_RUNPOD_PACKET (ST1 ornate + ST2 combat-active) and returns candidate PNGs for Claude to score. PARALLEL verify item (figure lane, not blocker): confirm existing faceless assets carry the Kitsune planar-geometry mask form per the option-(c) ruling. All reference-only; NO film/video/short/shotlist; NO canon/asset-lock; NO render by Claude.
