...
ZENITH_BLADE_FIGURE_FLAG = ENTITY spec defines Mikage = white B4C #FAFAFA shell + KITSUNE MASK WITH EYE SLITS (khe mat) + dark-red symmetric pattern + Enso ring; 3 phases. ⚠ CONFLICTS with recent "faceless sealed monocoque, no eye slits" figure work. NEEDS operator ruling in FIGURE LANE (entity spec §2). No figure files changed by Claude.
...
PRIOR_NEXT_TASK_DONE7 = RECORD_ENTITY_3PHASE_SPEC_AND_FIX_BLADE_DEVICE_SPEC (2026-06-01c; entity phase spec created; blade §1 device spec + §2 phase-scope corrected; packet -> ST1 ornate + ST2 combat-active; NO render by Claude)
CURRENT_NEXT_TASK = OPERATOR_RULE_FACELESS_VS_KITSUNE_AND_SUPPLY_BLADE_REFS — operator (1) RULES the figure-lane conflict: Kitsune-mask-with-eye-slits (master canon) vs faceless monocoque (recent work) — which is canon / how to reconcile; (2) selects the exact non-combat ornate MJ PNG(s) from Drive zenith / zenith V2; then optionally (3) runs ZENITH_BLADE_RUNPOD_PACKET (ST1+ST2) and returns candidate PNGs for Claude to score. All reference-only; NO film/video/short/shotlist; NO canon/asset-lock; NO render by Claude.
