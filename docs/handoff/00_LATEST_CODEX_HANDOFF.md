...
LATEST_COMPLETED_TASK = REVIEW_LIMITED_DOWNSTREAM_INTERNAL_ASSET_DECISION_WORK_FROM_LIMITED_GATE_V0_1
...
## NEXT SAFE TASK
NEXT_SAFE_TASK = PREPARE_LIMITED_INTERNAL_FOLLOW_UP_PLANNING_GATE_FROM_LIMITED_DOWNSTREAM_DECISION_WORK_V0_1

## NEXT TASK RULES
The next task may prepare a limited internal follow-up planning gate from the reviewed limited downstream decision work.

The next task must not create additional renders, edit PNG files, modify .blend files, create public output, deploy website/social assets, make final animation quality claims, make cinematic readiness claims, make final trailer claims, make public readiness claims, or make character completion claims.
