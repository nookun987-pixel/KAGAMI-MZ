# -*- coding: utf-8 -*-
r"""
MIKAGE BUILD LOG — STANDING HERO V0_4 (operator correction 2026-07-03):
(1) fixed a missing-glyph bug — the Space Mono font has no right-arrow glyph, so the CANVAS
    V0.2 note lines rendered as tofu boxes; replaced "->" for the arrow everywhere.
(2) added the LOOKDEV V0.8.1 hue-fix milestone (approved 2026-07-03, supersedes V0.8) as a
    third beat inside chapter 04, closing the "slit hue reads pink" flag left open by V0.8.

Still: FULL 0 -> final arc, rostrum-camera pans across multi-panel review sheets, REAL
per-round notes on a bottom scrim (transcribed from dispatch/ruling records, no invented status).

Supersedes v0_1/v0_2/v0_3 scripts (kept for history — do not run).
...
Run on the operator's machine:  python build_buildlog_standing_hero_v0_4.py
...
OUT   = os.path.join(HERE, "MIKAGE_BUILDLOG_STANDING_HERO_V0_4.mp4")
OUT_HOOK = os.path.join(HERE, "MIKAGE_BUILDLOG_STANDING_HERO_V0_4_HOOK.mp4")
...
def clip_pan(src, version, note1, note2, secs, w, out):
    """ROSTRUM CAMERA: sheet scaled near frame height (upscale capped 1.5x), camera pans
    across the panels so their printed labels stay readable. Eased pan (cosine)."""
    ov=os.path.join(w,"ov_%s.png"%os.path.basename(out)); notes_overlay(ov,version,note1,note2)
    im=Image.open(src); sw,shh=im.size
    th=min(H,int(shh*1.5)); tw=int(sw*th/shh)
    if tw<W: tw=W; th=int(shh*W/sw)         # narrow source: fill width instead
    th=min(th,int(H*1.7))                    # don't over-zoom very tall sheets
    tw,th=tw//2*2,th//2*2
    pw,ph=max(W,tw),max(H,th)
    D=max(secs-0.6,0.1)                      # ease finishes slightly before the cut
    ex="(iw-%d)*(0.5-0.5*cos(PI*min(t\\,%f)/%f))"%(W,D,D)   # \, — comma must be escaped inside a filter expr
    ey="(ih-%d)*(0.5-0.5*cos(PI*min(t\\,%f)/%f))"%(H,D,D)
    sh(["ffmpeg","-y","-v","error","-loop","1","-t",str(secs),"-i",src,"-loop","1","-t",str(secs),"-i",ov,
        "-filter_complex",
        "[0:v]scale=%d:%d,pad=%d:%d:(ow-iw)/2:(oh-ih)/2:color=0x050508,crop=%d:%d:%s:%s[sheet];"
        "[sheet][1:v]overlay=0:0,format=yuv420p[o]"%(tw,th,pw,ph,W,H,ex,ey),
