...
| **08** | **It Stands** | **FULL 0→final arc: blockout → match-to-master → helmet V0.3–0.7 → lookdev V0.8 → lookdev V0.8.1 hue fix → body V0.9–0.11 → assembly V0.12 → LOCK V0.14 → Canvas MOTION V0.2** | **NEXT — operator runs RUN_BUILDLOG_STANDINGHERO.bat (= v0_5 script, ~110s + hook). Operator rulings 2026-07-02: (1) build-log shows the PROCESS, not only finals; (2) review sheets are multi-panel — never paste a sheet whole. 2026-07-03 fixes: v0_3→v0_4 fixed a missing-glyph bug (Space Mono has no "→", rendered as tofu boxes) and added the V0.8.1 slit-hue-fix milestone. v0_4→v0_5: the rostrum pan still showed each sheet's OWN baked labels fighting with our caption ("text te le tua lua") — v0_5 crops every sheet down to ONE clean panel (hero_crop, skips the sheet's own label strip) and HOLDS it still (no pan/sweep) for the beat, so only our caption (version chip + the two real per-round notes) carries text, read like a calm lyric line.** |
