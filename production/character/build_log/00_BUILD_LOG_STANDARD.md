...
| **08** | **It Stands** | **FULL 0→final arc: blockout → match-to-master → helmet V0.3–0.7 → lookdev V0.8 → lookdev V0.8.1 hue fix → body V0.9–0.11 → assembly V0.12 → LOCK V0.14 → Canvas MOTION V0.2** | **NEXT — operator runs RUN_BUILDLOG_STANDINGHERO.bat (= v0_4 script, ~110s + hook). Operator rulings 2026-07-02: (1) build-log shows the PROCESS, not only finals; (2) review sheets are multi-panel — rostrum-camera pan so panels read full-size, with the REAL per-round FLAG/RULING notes on a scrim. Never paste a sheet whole. 2026-07-03 fix: v0_3 had a missing-glyph bug (Space Mono has no "→", rendered as tofu boxes on the CANVAS V0.2 notes) — v0_4 uses "->" instead, and adds the V0.8.1 slit-hue-fix milestone (approved 2026-07-03) as a 3rd beat inside chapter 04.** |
