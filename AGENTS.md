...
- Forty-seventh controlled exception is open:
  - `MIKAGE_CINEMATIC_LIGHTING_PASS_V0_1 = OPEN`
  - Exception #46 (robe locomotion cleanup) closed 2026-07-04. BOOS ruling 2026-07-04: the character reference sheet and hero lookdev currently read as a flat turntable-lit "sample," not a cinematic "scene" — violet reads as painted color rather than an emissive signal, and the void background reads empty rather than deep. BOOS supplied a 6-layer cinematic lighting spec (lighting / violet-emission / atmosphere / material / camera / grade); this task applies it as a NEW derivative, without touching geometry/rig/canon color.
  - Allowed base input (extend, do not rebuild): `production/character/production_actor/rig_derivatives/MIKAGE_HERO_LOOKDEV_PREMIUM_V0_8_1.blend` (ACTIVE_PREMIUM_LOOKDEV_REFERENCE, approved 2026-07-03). Full spec sheet: `production/character/build_log/MZ-CINEMATIC-PASS_REFERENCE_V0_1.html`. Full brief: `production/character/build_log/LANEA_CODEX_TASK_CINEMATIC_LIGHTING_PASS_V0_1.md`.
  - REQUIRED: (1) replace flat/even lighting with a single angled key (~40-50°) + low fill + negative fill + a cool rim/back light separating the cloak from the void background; (2) make the two slits true emission (`#8F00FF`), strength driven HIGH — target is "slits FULL violet": near-clip, strongly saturated, this is half of the shot's "wow" target per BOOS ruling 2026-07-04 ("khe full tim + halo glow trang = frame wow") — allowed to bounce a soft violet hint onto the porcelain cheeks only; (3) make the halo itself GLOW white strongly — increase the halo material's own emission strength (color stays white/near-white/cool-neutral, never shifted toward violet) plus selective bloom in the grade step so it visibly "blooms" in frame — this is the other half of the "wow" target, as strong as possible while the sampled pixel color stays pure white/near-white/cool-neutral; (4) add a very low-density volumetric haze (Codex chooses the engine approach and reports which/why) so rim/glow reads in the air; (5) give the porcelain helmet subtle SSS + non-uniform roughness + thin clearcoat, and the cloak a faint sheen/fresnel so rim catches its edge; (6) camera ~85mm, shallow DOF (f/1.4-2.8) focused on the slits, slight low angle, upper-third head placement; (7) grade with AgX or Filmic (report which), crush blacks toward `#050508`, lift only the violet band's saturation/luminance, selective bloom on BOTH the slits and the halo, very light CA on violet edges, fine grain, light vignette.
  - LOCKED (hard, non-negotiable): **halo stays absolute white/near-white/cool-neutral at every sampled pixel of every frame — zero violet, including from bounce/GI/rim/glow/bloom.** A strong white glow/bloom on the halo is REQUIRED (see item 3 above); a violet-tinted halo of any intensity is FORBIDDEN — "glow" means intensity, never a color shift. No geometry/rig/topology change. Canon base colors unchanged (helmet `#F2EEEA`, void black cloak, slit `#8F00FF`). No violet used as key/fill or washed across the whole scene. No teal-orange or any grade direction besides cool-mono + separated violet band. No blade animation, no new mesh/bone, no environment/set added.
  - SUCCESS: geometry/rig/topology unchanged from source (hash-confirm if possible); halo pixel-sampled at ≥4 points per frame across the contact sheet shows no violet drift (numeric RGB/hex reported, not just described); at least one AFTER frame/angle clearly shows BOTH the "wow" target elements together — slits reading strongly saturated/near-full violet AND the halo visibly glowing/blooming white; violet elsewhere appears only at the slits + soft cheek bounce + optional background bokeh; directional lighting with a working rim separates cloak from void; porcelain reads SSS/roughness-variation, not "plastic"; grade is cool-mono + separated violet, not teal-orange; contact sheet includes BEFORE (unmodified turntable render) and AFTER (cinematic) pairs at the same angle(s) for direct comparison; `python .mikage/tools/verify_output.py` prints PASS; no `.blend1` remains.
  - Allowed outputs (candidate only): `production/character/production_actor/rig_derivatives/MIKAGE_CINEMATIC_LIGHTING_PASS_V0_1.blend` + `..._V0_1_CONTACT_SHEET.png` + `..._V0_1_PROOF.md` (reviews); gate folder `_tmp/mikage_cinematic_lighting_pass_v0_1_gate/` holds ONLY `contact_sheet.png` + `contact_sheet_review_report.md`.
  - FAIL: any violet pixel detected on the halo at any sampled point/frame → `BLOCKER = HALO_COLOR_VIOLATION`; violet used as key/fill or washed across the environment → `BLOCKER = VIOLET_IN_ENVIRONMENT`; geometry/rig/topology changed outside lighting/material/camera/grade → `BLOCKER = SCOPE_VIOLATION` (report exactly what changed); canon base color (helmet/cloak/slit hue) drifts → `BLOCKER = CANON_COLOR_DRIFT`; gate mis-schema'd or missing the numeric halo-sample confirmation in the report → `BLOCKER = VALIDATOR_SCHEMA_MISMATCH`.
