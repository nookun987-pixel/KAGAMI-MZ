...
- Thirty-fourth controlled exception is open:
  ...
  - Final visual ruling belongs to the operator.
  - On SSOT conflict or scope drift: stop and report.
  - RESULT 2026-07-02: V0.1 = TECHNICAL_VALIDATION PASS (8 views clean, labels correct, gate clean, no mesh errors at new angles, no drift) but operator ruling = VISUAL_REFERENCE_GATE HOLD_FOR_FIX — figure fills only ~26-27% of each panel (too small to check detail) and 180 BACK / 225 / 270 LEFT are under-lit, coat merges into void. APPROVED_AS_MASTER_REFERENCE = NO. Fix pass opened below (Thirty-fifth exception, V0.2). This exception is now CLOSED / superseded by V0.2.

- Thirty-fifth controlled exception is open:
  - `MIKAGE_STANDING_HERO_TURNAROUND_V0_2 = OPEN`
  - V0.1 turnaround = TECHNICAL_VALIDATION PASS but VISUAL_REFERENCE_GATE = HOLD_FOR_FIX: figure fills only ~26-27% of each panel (too small to check silhouette/edge detail), and 180 BACK / 225 / 270 LEFT are lit flat enough that the coat merges into the void. This task = fix ONLY camera framing + fill/back lighting for those two problems, on the validated V0.1 setup. CAMERA + LIGHTING ONLY. NO geometry edit, NO material edit, NO object-transform change, NO camera-angle/rotation change. CANDIDATE only.
  - Only allowed next task: `MIKAGE_STANDING_HERO_TURNAROUND_V0_2`. Full brief: `production/character/build_log/LANEA_CODEX_TASK_STANDING_HERO_TURNAROUND_V0_2.md`.
  - Allowed base input (ONLY this): `production/character/production_actor/rig_derivatives/MIKAGE_STANDING_HERO_TURNAROUND_V0_1.blend` (keeps the validated V0.1 camera-rig/light-rig setup; only the two fixes below are added). Do NOT overwrite it. Report BASE_SELECTED + BODY_HASH_BEFORE + BODY_HASH_AFTER (must be identical).
  - LOCKED: ALL mesh geometry (BODY_HASH unchanged vs V0.1); character pose; ALL materials and material values (slits stay `#8F00FF` static); blade position; helmet + slit geometry; the 8 camera angles/rotation/order (only distance/orthographic scale may change); labels; render dimensions; base file hashes.
  - ALLOWED (camera + lighting only): reframe all 8 cameras (distance or orthographic scale per existing camera type, NOT rotation/angle/focal perspective) so full-figure height goes from ~26-27% to 78-82% of panel height (target 80%), identical scale and vertical alignment across all 8, keeping helmet + full body + blade + lowest geometry inside frame; add/adjust a neutral fill or back light so views 180, 225, 270 clearly read shoulder contour, torso volume, coat edges, lower silhouette, and blade separation, keeping void background `#050508`, not raising global exposure, not clipping the helmet, and not shifting slit hue.
